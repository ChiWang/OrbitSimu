...
typedef struct {
  ...
  /// in FT2 format
  std::string OutFile;  //NOTE:  not used in DAMPE
  /// Optional file used for ascii dump      
  std::string OptFile;  // NOTE: not used in DAMPE
  /// SAA file definition    
  std::string saafile;      
...
typedef struct {
   ...
   */
struct	AtTime{
  AtTime():yr(0),mo(0),dy(0),hr(0),mn(0),sc(0),ms(0){}
  /// year 
  int yr;
  /// month
  int mo; 	
...
public:
  void print()
  {
   std::cout<<yr<<"-"<<mo<<"-"<<dy<<"\t\t"<<hr<<":"<<mn<<":"<<sc<<":"<<ms<<std::endl;
  }
};// AtTime;
   ...
   */
struct atElemTle{
        // NOTE: input of *.tle file
  /// Time structure
  AtTime tm;
  /// Time stamp in MJD when the parameters are good from
  double mjd;
  /// Fraction of second of the time stamp (unit: minute!!)
  double isc;
  /// Satellite Number
  int SatNum,
  ...
  void print(){
    ...
    std::cout<<"meam anomaly = "<<xmo<<std::endl;
    std::cout<<"meam motion = "<<xno<<std::endl;
  }
};
